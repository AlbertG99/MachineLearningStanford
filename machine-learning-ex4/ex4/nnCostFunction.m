...
penalty_1 = sum(sum(Theta1(:, 2:end).^2, 2));
penalty_2 = sum(sum(Theta2(:, 2:end).^2, 2));
penalty = penalty_1 + penalty_2;

reg = lambda * penalty/(2 * m);
J = sum(sum((-Y) .* log(h) - (1-Y) .* log(1-h), 2))/m + reg;
...
Theta1_grad = delta_1./m + reg_1;
Theta2_grad = delta_2./m + reg_2;
